...
export function SessionCard({
  ...
  const accent = cancelled
    ? "#fca5a5"
    : activeBookings >= section.capacity
      ? "#f59e0b"
      : "#20BEF9";
  ...
  return (
    ...
    >
      <p className="mb-1 text-[10px] font-black uppercase tracking-[0.2em] text-[#20BEF9]">
        {formatSessionTime(section.StartTime)} -{" "}
        {formatSessionTime(section.endTime)}
      </p>
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3
            className={`text-lg font-black ${cancelled ? "text-slate-300" : "text-slate-800"}`}
          >
            {section.title}
          </h3>
      ...
      <button
        ...
        className="mt-3 flex items-center gap-1 text-xs font-bold text-slate-500 hover:text-[#20BEF9]"
      >
