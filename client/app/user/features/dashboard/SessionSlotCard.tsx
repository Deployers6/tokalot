...
export function SessionSlotCard({
  ...
  const spotsLeft =
    session.capacity != null ? session.capacity - bookingCount : null;
      ...
      >
        <div className="flex flex-col gap-3 px-4 py-4">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="mb-1 text-[10px] font-bold text-green-500">
                SCHEDULED
              </p>
              <p className="text-sm font-bold text-black">{session.title}</p>
              <p className="mt-0.5 text-xs text-slate-400">{session.level}</p>
            </div>
  ...
  return (
    <article className="flex items-center justify-between rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-sm">
      ...
      <button
        onClick={onBook}
        disabled={booking}
        className="ml-3 shrink-0 rounded-full bg-sky-500 px-4 py-2 text-xs font-bold text-white disabled:opacity-60"
      >
        {booking ? "..." : "Book Now"}
      </button>
    </article>
